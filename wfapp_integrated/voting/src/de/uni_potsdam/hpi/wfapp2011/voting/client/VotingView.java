...
public class VotingView extends Composite {
	...
	private void loadMaxVotes() {
		databaseService.numberOfVotes(Voting.type, Voting.semester, Voting.year, new AsyncCallback<Integer>(){
			...
			PasswordConfirmationPopUp pwPopUp = new PasswordConfirmationPopUp(new AsyncCallback<Boolean>() {
				...
				public void onSuccess(Boolean result) {
					if (!result) {
						...
						//create Vote objects for each vote
						...
						votes.add(new Vote(0, prio, i.getTopic().getProjectID(),i.getTopic().getName()));
						databaseService.saveVotes(Voting.type, Voting.semester, Voting.year, sessionID, votes,  new AsyncCallback<Void>(){
