...
public class LoginFilter implements Filter{
	...
	private String redirectUrl = "/voting/";
	private String url = "/login/";
	// TODO: Replace with correct URL
	private String type = ProcessInfo.getTypeFromURL("");
	private String semester = ProcessInfo.getSemesterFromURL("");
	private int year = ProcessInfo.getYearFromURL("");
				...
				if(result == null){
					...
					if(SessionManagement.getInstance().isLoggedIn(username)){
						...
						if(SessionManagement.getInstance().login(username, pwd, user.getValue())){
							...
							//TODO replace e-mail address generation
							try {
								db.connect(type, semester, year);
								
								//try if user is already in the database
								Collection<Map<String,String>> users = db.executeQuery("SELECT * FROM PERSON WHERE NAME ='"+username+"'");
								if (users.isEmpty()) {
									db.executeUpdate("INSERT INTO PERSON(NAME, EMAIL, ROLE) VALUES('"+username+"','"+username+"@student.hpi.uni-potsdam.de','"+LdapModul.getInstance().getUserdata(username)+"')");
								}
