...
/**
 * <code>EmailSender</code> is a JavaService-Task for the "activiti"-Engine,
 * which sends e-mails to all students(must have been logged in once) a few days
 * before the end of the voting phase. Students who have voted get a list with
 * their current voting. Students who have not voted yet get a reminder.
 * 
...
public class EmailSender implements JavaDelegate {
	...
	/**
	 * Fetches all students and their voting from the database and generates and
	 * sends the e-mails.
	 * 
	...
	public void execute(DelegateExecution arg0)
			throws UnsupportedEncodingException, SQLTableException {
		...
		try {
			...
			Collection<Map<String, String>> PersonsWithoutVoting = connection
					.executeQuery("SELECT NAME, EMAIL FROM PERSON "
			...
			for (Map<String, String> person : PersonsWithoutVoting) {
				...
				String emailText = "Sehr geehrte/r "
						+ name[0] + " "	+ name[1] + ",\n" + "\n"
						+ "Sie haben sich bei der Projektwahl " + Semester + " " + Year
						+ " angemeldet. Allerdings liegt noch keine Stimmenabgabe von Ihnen vor.\n"
				...
				SmtpEmailSender.getInstance().sendEmail(person.get("email"),
						name[0] + " " + name[1],
						"Projektwahl " + Semester + " " + Year, emailText);
			}
		...
		try {
			connection.connect(Type, Semester, Year);
			Collection<Map<String, String>> PersonsWithVoting = connection
					.executeQuery("SELECT NAME, EMAIL, PROJECTNAME, PRIORITY"
			...
			for (Map<String, String> person : PersonsWithVoting) {
				...
				VotingList.get(person.get("name")).add(
						person.get("projectname"));
			}
			...
			for (String person : VotingList.keySet()) {
				...
				String emailText = "Sehr geehrte/r " + name[0] + " " + name[1]
						+ ",\n" + "\n" + "Sie haben sich bei der Projektwahl "
						+ Semester + " " + Year + " angemeldet.\n"
						+ "Ihrer momentan Wahl ist wie folgt: \n";

				for (int i = 0; i < VotingList.get(person).size(); i++)
					emailText = emailText + "\n" + (i + 1) + ". "
							+ VotingList.get(person).get(i) + "\n";

				emailText = emailText
						+ "Bis zum "
						+ Enddate
						+ " k\u00F6nnen sie ihre Stimmenabgabe noch \u00E4ndern.\n"
						+ "\n"
						+ "Mit freundlichen Gr\u00FCen\n"
						+ "WFAPP2011 - Ihr Votingsystem\n\n"
						+ "Diese Email dient lediglich zur Information und muss nicht beantwortet werden.";

				SmtpEmailSender.getInstance().sendEmail(
						person + "@student.hpi.uni-potsdam.de",
						name[0] + " " + name[1],
						"Projektwahl " + Semester + " " + Year, emailText);
			}
			connection.disconnect();
