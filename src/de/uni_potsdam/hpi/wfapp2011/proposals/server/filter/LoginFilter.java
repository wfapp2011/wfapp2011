...
public class LoginFilter implements Filter{
	...
	private String redirectUrl = "/proposals/";
				...
				if(result == null){
					...
					if(SessionManagement.getInstance().isLoggedIn(username)){
						...
						if(SessionManagement.getInstance().login(username, pwd, user.getValue())){
							...
							//TODO replace e-mail address generation and procces id
							try {
								db.connect("Ba", "SS", 2014);
							...
							db.connectToMetaTables();
							try{
								if(debug) System.out.println("Alles Eintr�ge mit der ID l�schen");
