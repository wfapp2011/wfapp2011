...
public class ProjectEditPopUp extends MySimplePopUp {
	...
	@SuppressWarnings("deprecation")
	private boolean saveCheckedDate(DateBox box, Date oldValue, String name, Map<String,String> returnValues){
		boolean temp = false;		
		String value = "";
		
		if (box.getValue().compareTo(oldValue)>=0){
	...
	public Map<String,String> getContent(){
		...
		if (!saveCheckedDate(deadlineProposalCollection_dateBox,deadlineProposalCollection,"deadlineProposalCollection",returnValues)) return null;
		if (!saveCheckedDate(deadlineTopicsPublication_dateBox,deadlineTopicsPublication,"deadlineTopicsPublication",returnValues)) return null;
		if (!saveCheckedDate(deadlineVoting_dateBox,deadlineVoting,"deadlineVoting",returnValues)) return null;
		if (!saveCheckedDate(deadlineMatching_dateBox,deadlineMatching,"deadlineMatching",returnValues)) return null;
		if (!saveCheckedDate(deadlineProcess_dateBox,deadlineProcess,"deadlineProcess",returnValues)) return null;		
